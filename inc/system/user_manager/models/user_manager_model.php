...
class user_manager_model extends MY_Model {
	...
	*/
	...
	public function signup($fullname, $email, $password, $confirm_password, $timezone, $terms, $code, $number,$opt_verified){
		validate("min_length", __("Fullname"), $fullname, 5);
		validate("null", __("Email is required"), $email);
		validate("email", "", $email);
		validate("number", __("Phone Number is Required"), $number);
		...
		if(!isset($opt_verified)){

			ms([
				"status"  => "error",
				"message" => __('User is not verified through otp')
			]);
	    ...
	    $phone_number = $code.$number;
		$phone_number = preg_replace('/[^\dxX]/', '', $phone_number);
		$message = 'Welcome to Waziper your Login Username is "'.$email.'" and Password is "'.$password.'" Thanks for Joining Waziper';
		$theUrl= 'https://waziper.in/api/send.php?number='+phone_number+'&type=text&message='+message+'&instance_id=633BD9743BE7F&access_token=69d3dbbe3033342e91284c48efdefb81';
		$ch = curl_init();
		 ...
		 //echo print_r($api_response_arr['status']);
		  /*if($api_response_arr){
	...
	public function get_list($limit=-1, $page=-1)
	{
        ...
        if($role !='1'){	
			$this->db->where("a.created_by", $uid);
		}
		if($k)
		{
			$i = 1;
