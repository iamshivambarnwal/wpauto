...
class LicenseBoxExternalAPI{
	...
	public function __construct(){ 
		$this->product_id = '4A3F2960';
		$this->api_url = 'http://waziper.live/';
		$this->api_key = '6AE2001B6D27D39C5923';
	...
	public function verify_license($time_based_check = false, $license = false, $client = false){
		...
		if($time_based_check && $this->verification_period > 0){
			...
			$today = date('d-m-Y');
			if(empty($_SESSION["c7079e179f554c3"])){
				$_SESSION["c7079e179f554c3"] = '00-00-0000';
			}
			if($type == 1){
				$type_text = '1 day';
			}elseif($type == 3){
			...
			if(strtotime($today) >= strtotime($_SESSION["c7079e179f554c3"])){
				$get_data = $this->call_api(
					'POST',
